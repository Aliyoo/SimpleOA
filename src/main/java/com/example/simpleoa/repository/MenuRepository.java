...
public interface MenuRepository extends JpaRepository<Menu, Long> {
    ...
    // 根据用户ID查找有权限访问的菜单
    @Query("SELECT DISTINCT m FROM Menu m JOIN m.permissions p JOIN Role r ON p MEMBER OF r.permissions JOIN r.users u WHERE u.id = :userId ORDER BY m.sortOrder ASC")
    List<Menu> findMenusByUserId(@Param("userId") Long userId);

    // 根据角色ID查找有权限访问的菜单
    @Query("SELECT DISTINCT m FROM Menu m JOIN m.permissions p JOIN Role r ON p MEMBER OF r.permissions WHERE r.id = :roleId ORDER BY m.sortOrder ASC")
