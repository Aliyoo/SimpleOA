<template>
  <div class="holiday-container">
    <el-card class="box-card" shadow="always">
        ...
        <el-col :span="24">
          <el-form :inline="true" :model="queryForm" class="query-form">
            ...
            <el-form-item label="名称">
              <el-input v-model="queryForm.name" placeholder="输入节假日名称" style="width: 150px;" @keyup.enter="searchHolidays" />
            </el-form-item>
            <el-form-item>
      ...
      <el-table :data="holidays" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button @click="editHoliday(row)" type="primary" size="small">编辑</el-button>
            <el-button @click="deleteHoliday(row.id)" type="danger" size="small">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="addDialogVisible" title="添加节假日" width="500px">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleAddHoliday" :loading="loading">添加</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑节假日" width="500px">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleUpdateHoliday" :loading="loading">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function searchHolidays() {
  if (queryForm.name) {
    loading.value = true
    axios.get(`/api/holidays/search?name=${queryForm.name}`)
      .then(response => {
        holidays.value = response.data
      })
      .catch(error => {
...
function handleUpdateHoliday() {
  editFormRef.value.validate((valid) => {
    if (valid) {
      loading.value = true
      axios.put(`/api/holidays/${editForm.id}`, {
        date: editForm.date,
        name: editForm.name,
        type: editForm.type,
