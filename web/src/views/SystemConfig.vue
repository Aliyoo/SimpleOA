  ...
  <div class="system-config-container">
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="分类配置" name="categoryConfig">
        <el-row :gutter="20">
          ...
          <el-col :span="18">
            <div class="category-configs" v-if="selectedCategory">
              <h3>{{ selectedCategory }} 配置</h3>
              <el-form 
                :model="categoryConfigForm" 
                label-width="180px"
                class="category-config-form"
              >
                <el-form-item 
                ...
                <el-form-item>
                  <el-button 
                    type="primary" 
                    @click="saveCategoryConfigs"
                    :loading="savingCategoryConfigs"
                  >保存配置</el-button>
                </el-form-item>
              </el-form>
            </div>
    ...
    >
      <el-form :model="configForm" label-width="120px" :rules="configRules" ref="configFormRef">
        ...
        <el-form-item label="新分类名称" prop="newCategory" v-if="configForm.category === 'NEW'">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="submitConfigForm" :loading="submittingConfig">
            确定
          </el-button>
        </span>
      </template>
...
const isNumberConfig = (key) => key?.toLowerCase().includes('count') || key?.toLowerCase().includes('limit') || key?.toLowerCase().includes('size') || key?.toLowerCase().includes('duration') || key?.toLowerCase().includes('interval') || key?.toLowerCase().includes('port');
const isBooleanConfig = (key) => key?.toLowerCase().startsWith('is_') || key?.toLowerCase().startsWith('enable_') || key?.toLowerCase().includes('enabled') || key?.toLowerCase().includes('active');
