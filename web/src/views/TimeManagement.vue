      ...
      <div class="custom-tabs-content">
        ...
        <div v-show="activeTab === 'report'" class="tab-pane">
          <el-form :model="timeReportForm" label-width="100px" class="report-form">
            <el-form-item label="项目名称" prop="project">
              ...
              <div v-if="projectList.length === 0" class="form-item-hint">
                <el-alert
            ...
            <el-form-item label="工作内容" prop="description">
              <el-input v-model="timeReportForm.description" type="textarea" :rows="4" placeholder="请输入工作内容" />
            </el-form-item>
        ...
        <div v-show="activeTab === 'approval'" class="tab-pane">
          <div class="approval-container">
            ...
            <div class="search-bar">
              <el-form :inline="true" :model="approvalSearchForm" class="search-form">
                <el-form-item label="项目">
                  <el-select
                    v-model="approvalSearchForm.projectId"
                    placeholder="选择项目"
                    clearable
                    style="width: 200px"
                  >
            ...
            <!-- 查询结果提示 -->
            <div v-if="!approvalLoading && approvalList.length > 0" class="query-result-info">
              当前显示 <span class="highlight">{{ approvalList.length }}</span> 条记录， 时间范围：
              <span class="highlight">{{ approvalSearchForm.dateRange[0] || '-' }}</span> 至
              <span class="highlight">{{ approvalSearchForm.dateRange[1] || '-' }}</span>
            </div>

            <!-- 工时记录列表 -->
            <el-table v-loading="approvalLoading" :data="approvalList" style="width: 100%" :row-key="(row) => row.id">
              ...
              <el-table-column label="操作" width="180">
                <template #default="scope">
                  ...
                  <el-button v-if="!scope.row.approved" size="small" type="danger" @click="deleteTimeReport(scope.row)">
                    删除
                  </el-button>
                  <el-button size="small" type="info" @click="viewTimeReportDetail(scope.row)"> 详情 </el-button>
                </template>
              </el-table-column>
            </el-table>
              ...
              <div v-if="selectedTimeReport">
                <el-descriptions :column="2" border>
                  <el-descriptions-item label="项目">{{
                    selectedTimeReport.project?.name || '未知项目'
                  ...
                  <el-descriptions-item label="工作内容" :span="2">{{
                    selectedTimeReport.description
                  }}</el-descriptions-item>
                </el-descriptions>
              </div>
              <template #footer>
                <span class="dialog-footer">
                  ...
                  <el-button
                    v-if="selectedTimeReport && !selectedTimeReport.approved && isApprover"
                    type="success"
                    @click="approveTimeReport(selectedTimeReport)"
                    >通过</el-button
                  >
                  <el-button
                    v-if="selectedTimeReport && !selectedTimeReport.approved && isApprover"
                    type="warning"
                    @click="rejectTimeReport(selectedTimeReport)"
                    >驳回</el-button
                  >
                  <el-button
                    v-if="selectedTimeReport && !selectedTimeReport.approved"
                    type="danger"
                    @click="deleteTimeReport(selectedTimeReport)"
                    >删除</el-button
                  >
                </span>
              </template>
        ...
        <div v-show="activeTab === 'batch'" class="tab-pane">
          <div class="batch-container">
            ...
            <div v-loading="batchLoading" class="batch-table-container">
              <!-- 项目数据表格 -->
              <el-table
              ...
              <!-- 空状态显示 -->
              <div v-if="batchProjects.length === 0 && !batchLoading" class="batch-empty">
                <el-empty description="暂无项目数据">
                  <template #description>
                    <div class="empty-description">
                      <p v-if="projectLoadingState.hasError">获取项目数据失败，请检查网络连接或联系管理员</p>
                      <p v-else-if="projectLoadingState.retryCount >= projectLoadingState.maxRetries">
                        已尝试多次获取项目数据，请联系管理员确认您的项目权限
                      </p>
                      <p v-else>当前没有可用的项目数据</p>
                    </div>
                  </template>
                  <template #extra>
                    <el-button
                      ...
                      :disabled="
                        ...
                        projectLoadingState.retryCount >= projectLoadingState.maxRetries
                      "
              ...
              <!-- 项目加载状态提示 -->
              <div v-if="!batchLoading && batchProjects.length > 0" class="batch-status-info">
                <el-alert
                  v-if="
                    projectLoadingState.retryCount > 0 &&
                    projectLoadingState.retryCount < projectLoadingState.maxRetries
                  "
          ...
          <div class="statistics-container">
            <div class="statistics-header">
              <div class="statistics-filters">
                ...
                >
                  <el-option key="all" label="所有项目" value="" />
                  <el-option
                    v-for="project in projectList"
                    :key="project.id"
                    :label="project.name"
              ...
              <el-button
                type="primary"
                :disabled="!statisticsDateRange || statisticsDateRange.length !== 2"
...
const fetchStatisticsData = async () => {
  ...
  if (
    ...
    statisticsDateRange.value.length !== 2 ||
    !currentUser.value ||
    ...
    if (!projectList.value || projectList.value.length === 0) {
      ...
      try {
        ...
        if (projectList.value && projectList.value.length > 0) {
          ...
          const now = Date.now()
          if (
            !batchState.value.hasShownEmptyWarning ||
            !projectLoadingState.value.lastErrorTime ||
            now - projectLoadingState.value.lastErrorTime > projectLoadingState.value.errorCooldown
          ) {
...
onMounted(async () => {
  ...
  if (currentUser.value && currentUser.value.role) {
    ...
    const roleStr = Array.isArray(currentUser.value.role)
      ? currentUser.value.role.join(',')
